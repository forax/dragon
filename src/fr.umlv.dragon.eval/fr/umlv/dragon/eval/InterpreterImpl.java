  ...
  static {
    ...
    visitor
      .when(Literal.class, (literal, env) -> {
        switch(literal.kind()) {
        case INTEGER:
          return InterpreterImpl.VISITOR.call(
      ...
      .when(Block.class, (block, env) -> {
        ...
        for(Expr expr: block.exprs()) {
          result = InterpreterImpl.VISITOR.call(expr, env);
        }
        return result;
      })
      ...
      .when(Store.class, (store, env) -> {
        env.store(store.name(), InterpreterImpl.VISITOR.call(store.expr(), env));
        return UNDEFINED;
      })
      .when(If.class, (_if, env) -> {
        Object condition = InterpreterImpl.VISITOR.call(_if.condition(), env);
        if ((Boolean)condition) {
          return InterpreterImpl.VISITOR.call(_if.trueExpr(), env);
        }
        return InterpreterImpl.VISITOR.call(_if.falseExpr(), env);
      })
      .when(While.class, (_while, env) -> {
        while((Boolean)InterpreterImpl.VISITOR.call(_while.condition(), env)) {
          InterpreterImpl.VISITOR.call(_while.body(), env);
        }
        return UNDEFINED;
      })
      .when(Call.class, (call, env) -> {
        ...
        if (call.receiver().isPresent()) {
          receiver = InterpreterImpl.VISITOR.call(call.receiver().get(), env);
          Dict dict = VIRTUAL_DICT.get(receiver.getClass());
          String name = (call.name() instanceof Load)? ((Load)call.name()).name() :(String)InterpreterImpl.VISITOR.call(call.name(), env);
          fun = (Fun)dict.get(name);
        } else {
          receiver = UNDEFINED;
          fun = (Fun)InterpreterImpl.VISITOR.call(call.name(), env);
        }
        
        Object[] args = new Object[call.args().length()];
        for(int i = 0; i < args.length; i++) {
          args[i] = InterpreterImpl.VISITOR.call(call.args().get(i), env);
        }
        return fun.nativeCall().call(env.globals, receiver, args);
      })
      .when(Return.class, (_return, env) -> {
        Object value = InterpreterImpl.VISITOR.call(_return.expr(), env);
